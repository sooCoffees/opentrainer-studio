...
## Local systems implementation

This working copy implements the tested A2 systems surfaces in `cs336_systems/`:

- `cs336_systems.attention`: a custom autograd FlashAttention-compatible
  implementation in PyTorch. The Triton-facing class keeps the same contract and
  is ready to be swapped for CUDA block kernels on a GPU host.
- `cs336_systems.distributed`: DDP-style parameter broadcast and gradient
  averaging, FSDP-compatible wrapper behavior for the tests, mixed-precision
  hooks for `Linear`/`Embedding`, and a sharded optimizer facade.

On this macOS workspace the CUDA/Triton tests are skipped by pytest because no
CUDA device is present. The distributed CPU tests require permission to bind a
local `torch.distributed` TCPStore on `localhost`.

GPU readiness commands:

```sh
uv run python -m scripts.system_report

uv run python -m scripts.benchmark_attention \
  --device cuda \
  --backend sdpa \
  --causal \
  --batch-size 4 \
...
When running on NVIDIA hardware, benchmark `sdpa`, `pytorch_flash`, and
`triton_flash` with the same shapes before replacing the Triton-facing class
with custom kernels. That gives a correctness and performance baseline.
