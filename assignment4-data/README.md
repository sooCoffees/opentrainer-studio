...
This workspace adds `cs336_data.pipeline` and wires the tests through
`tests/adapters.py`. The local pipeline currently supports:

- HTML-to-text extraction with block-aware formatting.
- English/Chinese heuristic language detection.
- Email, phone, and IP masking.
- Lightweight NSFW/toxicity and Gopher-style quality filters.
- Exact line deduplication and fuzzy document deduplication.

Next GPU/data-scale improvements should be added behind the same adapter
interfaces: CLD3/fastText language ID, learned quality classifiers, MinHash LSH,
streaming dataset manifests, and tokenizer-aware shard writing.
