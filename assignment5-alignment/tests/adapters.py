...
from cs336_alignment.core import (
...
def run_grpo_train_step(
    ...
    advantages, _, adv_metadata = compute_group_normalized_rewards(
        raw_rewards, group_size, baseline, advantage_eps, advantage_normalizer
    )
    ...
    labels = tokenized["labels"].to(input_ids.device)
    response_mask = tokenized["response_mask"].to(input_ids.device)
    advantages = advantages.to(input_ids.device)
    optimizer.zero_grad(set_to_none=True)
    log_probs = get_response_log_probs(model, input_ids, labels, False)["log_probs"]
    per_token_loss, loss_metadata = compute_policy_gradient_loss(
        advantages,
        log_probs,
        importance_reweighting_method,
        old_log_probs.to(input_ids.device) if old_log_probs is not None else None,
        cliprange,
        response_mask,
    )
    ...
    optimizer.step()
    optimizer.zero_grad(set_to_none=True)
    return loss.detach(), metadata | adv_metadata | loss_metadata | {
